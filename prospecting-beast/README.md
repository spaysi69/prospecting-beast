...
- `POST /api/tools/xray-matrix` — generates targeted public-search queries using F/NF roles, location and industry.
- `POST /api/tools/email-candidates` — generates common corporate email patterns and checks MX records.
- `POST /api/tools/public-page-parse` — fetches permitted public pages with `aiohttp`, respects `robots.txt`, and parses JSON-LD Person records.

The public-page parser deliberately refuses direct LinkedIn profile crawling. Use publicly indexed search results or an approved LinkedIn API instead.
