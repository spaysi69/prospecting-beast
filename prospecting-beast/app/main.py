...
PROGRESS_LOCKS: dict[str, asyncio.Lock] = {}
...
class JobIn(BaseModel):
    ...
    mode: str = 'f'
    max_people_per_company: int = Field(default=20, ge=1, le=200)
...
async def process_company(job_id, family_id, root_domain, domain, relationship, parent_domain, cfg, seamless_pool, family_state, lead_cache, progress_cb=None):
    ...
    max_people = max(1, min(int(cfg.get('max_people_per_company', 20) or 20), 200))
    # Existing rows count toward the hard company quota. This prevents a resumed job
    # from performing new web searches after the database already has its allowance.
    existing_ids={}
    for row in lead_cache.values():
        if row and row.get('domain') == domain and row.get('id'):
            existing_ids.add(str(row['id']))
    existing_count=len(existing_ids)
    if existing_count >= max_people:
        await append_log(job_id, f'[{domain}] QUOTA ALREADY FULL — {existing_count}/{max_people}; no web searches or family research.', 'warning')
        await prog(phase='completed', current_percent=100, detail=f'Quota full: {existing_count}/{max_people} leads already saved', candidates=existing_count, leads_saved=existing_count)
        await sb_update('companies', {'id':company_id}, {'people_found':existing_count,'qualified_people':existing_count,'status':'done','updated_at':now})
        return {'related': [], 'saved': 0}

    remaining_slots=max_people-existing_count
    await prog(phase='discovery', current_percent=8, detail=f'Starting public-web discovery · quota {existing_count}/{max_people}', queries_done=0, queries_total=int(cfg.get('web_search_budget',6) or 6), candidates=0, leads_saved=existing_count)
    company_name = company.get('name') or domain
    await append_log(job_id, f'[{domain}] WEB: public people discovery (need ≤{remaining_slots} more)…', 'info')
    ...
    if remaining_tavily <= 0:
        ...
        web = await web_people_discovery(
            company_name, domain, cfg['mode'], remaining_slots, max_queries,
            cfg.get('location',''), cfg.get('industry',''), bool(cfg.get('agentic_discovery',False)),
            progress_cb=prog, job_id=job_id
        )
    ...
    qcount = sum(1 for p in people if base_score(p.get('title') or p.get('job_title') or '', cfg['mode']))
    await append_log(job_id, f'[{domain}] WEB OK — {len(people)} candidates; {qcount} pass title matching.', 'success' if people else 'warning')
    await prog(phase='scoring', current_percent=62, detail=f'Scoring {len(people)} candidates · {qcount} qualified', queries_done=int(web.get('query_count',0) or 0), queries_total=int(max_queries if remaining_tavily>0 else 0), candidates=len(people), leads_saved=existing_count)
    ...
    people=normalized
    ranked = rank_people(people, cfg['mode'], min(remaining_slots, len(people) or 0))
    now_iso=__import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat()
    await sb_update('companies', {'id':company_id}, {'people_found':len(people),'qualified_people':len(ranked)+existing_count,'updated_at':now_iso})

    saved_here=0
    await prog(phase='saving', current_percent=70, detail=f'Saving up to {remaining_slots} new leads', candidates=len(people), leads_saved=existing_count)
    for _, info, raw_person in ranked:
        # HARD QUOTA: never save beyond max_people_per_company.
        if existing_count + saved_here >= max_people:
            await append_log(job_id, f'[{domain}] HARD QUOTA REACHED — {existing_count+saved_here}/{max_people}; stopping lead saves and further web research.', 'warning')
            break
        if await job_stop_requested(job_id):
        ...
        if await save_lead(job_id,family_id,company_id,person,lead_cache):
            saved_here += 1
            async with family_state.get('lock', asyncio.Lock()):
                family_state['leads_saved'] = family_state.get('leads_saved',0) + 1
            await prog(phase='saving', current_percent=min(89, 70 + (saved_here/max(1,remaining_slots))*18), detail=f'Saved {existing_count+saved_here}/{max_people} company leads', candidates=len(people), leads_saved=existing_count+saved_here)
            if excluded:
    ...
    # Critical: once the people quota is reached, no more Tavily calls are allowed,
    # including corporate-family discovery for this company.
    if existing_count + saved_here >= max_people:
        await append_log(job_id, f'[{domain}] QUOTA COMPLETE — {existing_count+saved_here}/{max_people}; family search skipped to prevent further Tavily usage.', 'success')
        await prog(phase='completed', current_percent=100, detail=f'Quota complete · {existing_count+saved_here}/{max_people} leads', leads_saved=existing_count+saved_here)
        ...
        return {'related': [], 'saved': saved_here}
    ...
    # Corporate family discovery is only attempted while the people quota is still open.
    await prog(phase='family', current_percent=92, detail='Mapping corporate family…', candidates=len(people), leads_saved=existing_count+saved_here)
    job_now = await get_job(job_id)
    st_now = (job_now or {}).get('stats') or {}
    if int(st_now.get('tavily_estimated_credits', 0) or 0) >= int(cfg.get('tavily_job_budget', 40)):
        ...
        await prog(phase='completed', current_percent=100, detail=f'Family search skipped — Tavily budget exhausted', leads_saved=existing_count+saved_here)
        return {'related': related, 'saved': saved_here}
    ...
    await prog(phase='completed', current_percent=100, detail=f'Company complete · {existing_count+saved_here} saved · {len(related)} family candidates', leads_saved=existing_count+saved_here)
    return {'related': related, 'saved': saved_here}
...
async def process_job(jid: str, cfg: dict):
    try:
        ...
        progress_lock=PROGRESS_LOCKS.setdefault(jid, asyncio.Lock())

        async def progress_update(*, root_index, root_total, company_index=0, company_done=0, company_active=1, **kw):
            async with progress_lock:
                p={
                    'phase':kw.get('phase','initializing'),
                    'current_percent':round(float(kw.get('current_percent',0) or 0),1),
                    'root_index':root_index,
                    'root_total':root_total,
                    'company_index':company_index,
                    'companies_processed':total_companies,
                    'company_queue':company_active,
                    'current_company':kw.get('current_company') or existing_stats.get('current_company') or '',
                    'queries_done':int(kw.get('queries_done',0) or 0),
                    'queries_total':int(kw.get('queries_total',0) or 0),
                    'candidates':int(kw.get('candidates',0) or 0),
                    'leads_saved':int(kw.get('leads_saved',total_leads) or 0),
                    'detail':kw.get('detail') or '',
                }
                # Each root occupies one stable segment of the job. This avoids progress
                # jumping backward when discovery dynamically finds more subsidiaries.
                root_fraction = (max(0, root_index-1) + p['current_percent']/100.0) / max(1, root_total)
                p['overall_percent']=round(min(100,max(0,root_fraction*100)),1)
                job_now=await get_job(jid)
                st=dict((job_now or {}).get('stats') or {})
                st['progress']=p
                st.update({'root_companies':root_total,'companies_processed':total_companies,'leads_saved':total_leads,'current_company':p['current_company']})
                await update_job(jid,stats=st)
        ...
        for root in roots:
            ...
            queue=[]; enqueued=set(completed)
            if rows:
                for c in rows:
                    status=(c.get('status') or 'queued').lower()
                    cd=c.get('domain')
                    if cd and cd not in completed and status in ('queued','processing','error','stopped'):
                        queue.append((cd,c.get('relationship','related'),c.get('parent_domain'),1.0))
                        enqueued.add(cd)
                if not queue and root not in completed:
                    queue=[(root,'original',None,1.0)]; enqueued.add(root)
            else:
                queue=[(root,'original',None,1.0)]; enqueued.add(root)
            family_state={'credits_used':int((cfg.get('family_credit_usage') or {}).get(root,0) or 0),'leads_saved':0,'lock':asyncio.Lock()}
            family_companies=0
            last_domain=root
            await append_log(jid, f'Starting family {root} ({root_index}/{len(roots)}) — {len(queue)} companies queued.' if rows else f'Starting family {root} ({root_index}/{len(roots)})')
            await progress_update(root_index=root_index, root_total=len(roots), company_index=family_companies, company_active=len(queue), current_company=root, phase='initializing', current_percent=0, detail=f'Family {root} · queue {len(queue)}')

            # Priority frontier: process the highest-confidence / queue-score companies
            # first, with a hard semaphore to keep Tavily load bounded.
            semaphore=asyncio.Semaphore(3)
            while queue and family_companies < int(cfg['max_companies_per_family']):
                if await job_stop_requested(jid):
                    await mark_job_stopped(jid, f'Family {root} stopped by user.')
                    return
                queue.sort(key=lambda x:x[3], reverse=True)
                batch=[]
                while queue and len(batch)<3 and family_companies+len(batch)<int(cfg['max_companies_per_family']):
                    batch.append(queue.pop(0))
                family_companies += len(batch)
                total_companies += len(batch)
                last_domain=batch[-1][0] if batch else last_domain
                await update_stats(jid, root_companies=len(roots), companies_processed=total_companies, current_company=', '.join(x[0] for x in batch), current_root=root, leads_saved=total_leads)

                async def run_one(item, local_index):
                    domain, relationship, parent_domain, queue_score=item
                    async with semaphore:
                        await append_log(jid, f'[{domain}] CONCURRENT SLOT {local_index+1}/{len(batch)} — processing.', 'info')
                        async def cb(**kw):
                            await progress_update(root_index=root_index, root_total=len(roots), company_index=family_companies, company_active=len(queue)+len(batch), current_company=domain, **kw)
                        try:
                            out=await process_company(jid,family_id,root,domain,relationship,parent_domain,cfg,pool,family_state,lead_cache,progress_cb=cb)
                            if isinstance(out, dict):
                                return {'domain':domain,'related':out.get('related') or [],'saved':int(out.get('saved',0) or 0),'ok':True}
                            return {'domain':domain,'related':out or [],'saved':0,'ok':True}
                        except Exception as e:
                            ...
                            return {'domain':domain,'related':[],'ok':False}

                results=await asyncio.gather(*(run_one(item,i) for i,item in enumerate(batch)), return_exceptions=False)
                total_leads += sum(int(r.get('saved',0) or 0) for r in results)
                added_total=0
                for result in results:
                    domain=result['domain']; related=result['related']
                    for rr in related:
                        rd=norm_domain(rr.get('domain',''))
                        conf=float(rr.get('confidence',0) or 0)
                        rel=(rr.get('relationship') or 'related').lower().strip()
                        queue_score=float(rr.get('queue_score') or conf*100)
                        await save_relationship(jid,family_id,domain,rr)
                        min_conf=float(env('MIN_RELATIONSHIP_CONFIDENCE','0.80'))
                        if not rd or rd==domain or rd in completed or rd in enqueued or conf <= min_conf:
                            continue
                        if added_total >= int(cfg['max_related_per_company']):
                            continue
                        enqueued.add(rd)
                        queue.append((rd,rel,domain,queue_score))
                        await find_or_create_company(jid,family_id,root,rd,rel,domain,rr.get('company') or rr.get('name'))
                        added_total += 1
                        await append_log(jid, f'[{domain}] QUEUED subsidiary/related {rd} · confidence {conf:.2f} · priority {queue_score:.1f}.', 'info')
                for item in batch:
                    completed.add(item[0])
                await progress_update(root_index=root_index, root_total=len(roots), company_index=family_companies, company_active=len(queue), current_company=last_domain, phase='family', current_percent=96 if not queue else 70, detail=f'Family frontier: {len(queue)} companies queued · {added_total} new relationships')

            total_leads = int((await get_job(jid) or {}).get('stats',{}).get('leads_saved',total_leads) or total_leads)
            ...
            await progress_update(root_index=root_index, root_total=len(roots), company_index=family_companies, company_active=0, current_company=last_domain, phase='completed', current_percent=100, detail=f'Family complete · {family_companies} companies processed · {len(queue)} left unprocessed')
            await append_log(jid,f'Family {root} complete. Companies processed: {family_companies}. Leads saved this family: {family_state.get("leads_saved",0)}.')
        ...
        st.update({'root_companies':len(roots),'companies_processed':total_companies,'seamless_credits_used':total_credits,'leads_saved':total_leads,
                   'progress':{**(st.get('progress') or {}),'phase':'completed','current_percent':100,'overall_percent':100,'detail':'Job complete'}})
...
async def test_web_discovery(payload: dict, _: bool = Depends(auth)):
    ...
    max_people = max(1, min(int(payload.get('max_people',20)), 200))
    if not domain:
