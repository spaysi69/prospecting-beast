...
# Corporate-family queue priorities. Higher values are processed first.
# The confidence score returned by Gemini is combined with this relationship
# weight so stronger corporate links reach the prospecting queue first.
RELATIONSHIP_QUEUE_PRIORITY = {
    'subsidiary': 100,
    'acquired_target': 95,
    'parent': 92,
    'sister': 82,
...
def relationship_queue_score(item: dict) -> float:
    try:
        confidence = max(0.0, min(1.0, float(item.get('confidence') or 0.0)))
    except (TypeError, ValueError):
        confidence = 0.0
    rel = str(item.get('relationship') or 'related').lower().strip()
    priority = RELATIONSHIP_QUEUE_PRIORITY.get(rel, RELATIONSHIP_QUEUE_PRIORITY['related'])
    # Confidence dominates; relationship type breaks close ties.
    return round(confidence * 100.0 * 0.75 + priority * 0.25, 2)
...
async def gemini_extract_people(company: str, domain: str, evidence: list[dict], mode: str) -> list[dict]:
    ...
    async with await http_client() as c:
        try:
            ...
            obj=json.loads(r.json()['candidates'][0]['content']['parts'][0]['text']); return obj.get('people',[]) if isinstance(obj,dict) else []
...
async def web_people_discovery(company: str, domain: str, mode: str, max_people: int=10, search_budget: int=8, location: str="", industry: str="", agentic: bool=False, progress_cb=None, job_id: str|None=None) -> dict:
    """Web-first employee discovery with a hard qualified-person ceiling.

    Progress is reported after every meaningful phase/search/extraction so the UI can
    show genuine live progress rather than a fake timer. The callback is optional to
    preserve the existing API used by tests/tools.
    """
    max_people = max(1, min(int(max_people or 20), 200))
    search_budget = max(1, min(int(search_budget or 8), 16))
    qbase = (company or '').strip() or domain
    evidence=[]; people=[]; extracted_keys=set()

    async def prog(**kw):
        if progress_cb:
            await progress_cb(job_id=job_id, **kw)
    ...
    await prog(phase='discovery', current_percent=10, detail='Crawling first-party company pages…', queries_done=0, queries_total=search_budget, candidates=0)
    ...
    if evidence:
        add_people(await gemini_extract_people(qbase, domain, evidence, mode))
        await prog(phase='discovery', current_percent=30, detail=f'Official pages extracted · {qualified_count(people)}/{max_people} qualified', queries_done=0, queries_total=search_budget, candidates=len(people))
        # Absolute quota stop: no subsequent Tavily calls.
        if qualified_count(people) >= max_people:
            qualified=people[:]
            await prog(phase='completed', current_percent=100, detail=f'Quota reached from official pages · {max_people}/{max_people}', queries_done=0, queries_total=search_budget, candidates=len(people))
            return {'people':qualified[:max_people],'evidence_count':len(evidence),'query_count':0,'estimated_tavily_credits':0,'target_terms':list(title_dictionary(mode).keys()),'sources_used':['official']}

    evidence.extend(await spiderfoot_osint(domain)); evidence.extend(await amass_passive_osint(domain))
    ...
    for i,item in enumerate(queries):
        # Check quota BEFORE a Tavily call.
        if qualified_count(people) >= max_people:
            break
        ...
        await prog(phase='discovery', current_percent=32 + (i/max(1,len(queries)))*24, detail=f'Search {i+1}/{len(queries)} · {q[:90]}', queries_done=completed_queries, queries_total=len(queries), candidates=len(people))
        try:
        ...
        for x in d.get('results',[]) or []:
            ...
            if score and score < 0.15: continue
            seen_urls.add(u)
            evidence.append({'title':x.get('title',''),'url':u,'content':(x.get('content') or '')[:6000],'score':score,'query':q})
        completed_queries += 1

        # Re-extract after every search, and immediately enforce the hard limit.
        batch=await gemini_extract_people(qbase, domain, evidence[-32:], mode)
        add_people(batch)
        qualified=qualified_count(people)
        await prog(phase='discovery', current_percent=min(60, 34 + (completed_queries/max(1,len(queries)))*24), detail=f'Search {completed_queries}/{len(queries)} · {qualified}/{max_people} qualified · {len(people)} candidates', queries_done=completed_queries, queries_total=len(queries), candidates=len(people))
        if qualified >= max_people:
            break

    qualified=[]; seen=set()
    for p in people:
        name=(p.get('name') or p.get('full_name') or '').strip(); title=(p.get('title') or p.get('job_title') or '').strip(); li=(p.get('linkedin_url') or '').strip()
        if not name or not title: continue
        info=base_score(title,mode)
        if not info: continue
        k=(li.lower() or name.lower(),normalize_title(title),domain)
        if k in seen: continue
        seen.add(k); p['name']=name; p['title']=title; p['linkedin_url']=li; p['_source']='public_web'; p['_source_url']=p.get('source_url') or ''
        p['_search_score']=max([float(x.get('score') or 0) for x in evidence if x.get('url')==p.get('_source_url')] or [0])
        qualified.append(p)
    qualified.sort(key=lambda p:((base_score(p.get('title',''),mode) or {}).get('score',0),(base_score(p.get('title',''),mode) or {}).get('similarity',0),p.get('_search_score',0)),reverse=True)
    await prog(phase='scoring', current_percent=66, detail=f'{len(qualified)} qualified candidates ready to save', queries_done=completed_queries, queries_total=len(queries), candidates=len(people))
    return {'people':qualified[:max_people],'evidence_count':len(evidence),'query_count':completed_queries,'estimated_tavily_credits':sum(2 if (item[1]=='linkedin' and i<2) else 1 for i,item in enumerate(queries[:completed_queries])),'target_terms':list(title_dictionary(mode).keys()),'sources_used':sorted(set((e.get('source') or e.get('query') or 'web').split(':')[0] for e in evidence))}
...
class SeamlessPool:
    ...
    async def research(self, person: dict):
        ...
        contact = {'contactName': (person.get('name') or '').strip(), 'domain': (person.get('domain') or '').strip()}
        # Seamless accepts contactName + domain (optionally title). Avoid sending
        # empty optional identifiers such as liProfileUrl, which can trigger 422.
        if person.get('title'):
            contact['title'] = str(person.get('title')).strip()
        if person.get('company'):
            contact['companyName'] = str(person.get('company')).strip()
        if person.get('email'):
            contact['email'] = str(person.get('email')).strip()
        if person.get('linkedin_url'):
            contact['liProfileUrl'] = str(person.get('linkedin_url')).strip()
        payload={'contacts':[contact], 'isJobChange': False}
            ...
            try:
                async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as c:
                    ...
                    if r.status_code in (401,403,429,402,422):
                        try:
                            errj=r.json()
                            detail = json.dumps(errj, ensure_ascii=False)
                        except Exception:
                            detail = (r.text or '').strip()
                        detail = re.sub(r'\s+', ' ', detail)[:400]
                        # The current Seamless API documents 422 as insufficient credits
                        # or missing license. Try another configured key, but retain the
                        # provider's response in the error so the UI shows the real cause.
                        self.bad.add(idx)
                        last_err=f'Seamless key {idx+1} unavailable: HTTP {r.status_code}' + (f' — {detail}' if detail else '')
                        continue
                    r.raise_for_status()
